	...
	public function add() {
		...
		if ($product_info) {
			...
			if (!$json) {
				...
				$popupcheck = $this->config->get('popupupsell');
				if(empty($popupcheck)) {
					$popupUpsellEnabled = false;
				} else {
                  $this->load->model('module/popupupsell');
                  $popupUpsellEnabled = $this->model_module_popupupsell->upsellWilBeShown($this->request->post['product_id']);
				}
				...
				if ( isset($BuyNow) && ($BuyNow['BuyNow']['Enabled'] == 'yes') ) {
					if ( $BuyNow['BuyNow']['ShowWhere']=='all_products' ) {
						...
						if($popupUpsellEnabled) {
							$this->session->data['buynow_redirect'] = $json['redirect'];
							$json['redirect'] = '';	
						}
...
// Tasawk Changes
        $json['text_count'] =  $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0);
